        ...
        if (typeof newUserShape[currUserShapeIdx][propertyTitle] === 'number') {
            ...
            if (sign === '+') {
                if (valueIndex === propertiesValueStrings[propertyTitle].length - 1) {
                    ...
                    newUserShape[currUserShapeIdx][propertyTitle] = propertiesValueStrings[propertyTitle][newValueIndex]
                } else {
                ...
                if (valueIndex === 0) {
                    const newValueIndex = propertiesValueStrings[propertyTitle].length - 1
                    newUserShape[currUserShapeIdx][propertyTitle] = propertiesValueStrings[propertyTitle][newValueIndex]
                } else {
                    const newValueIndex = valueIndex - 1
                    newUserShape[currUserShapeIdx][propertyTitle] = propertiesValueStrings[propertyTitle][newValueIndex]
    ...
    return (
        <>
            <_SC_flexColumn isCentered={true}>
                ...
                <ShapeContainer>
                    <Shape shapeProperties={targetShape} defaultProperties={levelsData[currIdxLevel]?.defaultProperties}
                        renderingKey={levelsData[currIdxLevel]?.renderingKey}
                    />
                </ShapeContainer>
                <br />
                <ShapeContainer>
                    <Shape shapeProperties={userShape} defaultProperties={levelsData[currIdxLevel]?.defaultProperties}
                        renderingKey={levelsData[currIdxLevel]?.renderingKey}
                    />
                </ShapeContainer>
                <br />
                <_SC_flexRow justifyContent='center' alignItems='center'>
                    {isVictory ? (
                        ...
                        (<>
                            <TriangleButton deg={degObj.LEFT} onClick={() => changeUserProperty(-1)} />
                            <PropertyContainer><PropertyName>{
                                propertyTitle
                            }</PropertyName>
                                <PropertyValue>{propertyValue}</PropertyValue>
                            </PropertyContainer>
                            <TriangleButton deg={degObj.RIGHT} onClick={() => changeUserProperty(1)} />
                        </>)}
                ...
                <_SC_flexRow justifyContent='space-between' alignItems='center'>
                    {isVictory ? '' :
                        (
                            <>
                                <CircleButton onClick={() => changeUserValues('-')}><Line degree={90} /></CircleButton>
                                <CircleButton onClick={() => changeUserValues('+')}><Line degree={90} /><Line degree={180} /></CircleButton>
                            </>
                        )}
                </_SC_flexRow>
